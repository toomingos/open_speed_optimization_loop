  ...
  return (
    <g>
      {labelsToShow.map((idx) => {
        ...
        const barX = barScale(point.loop) ?? 0;
        const barY = yScale(point._ms) ?? 0;

        return (
          <text
            key={`label-${point.loop}`}
            x={barX + bandWidth / 2}
            y={barY - 16}
            textAnchor="middle"
            className="font-pixel-square text-sm fill-foreground font-semibold tabular-nums"
          >
            {point._ms.toLocaleString()}ms
          </text>
        );
      })}
    </g>
...
export function CaseStudySummaryChart({ data }: Props) {
  ...
  return (
    <div>
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-1.5">
              <svg width="12" height="12" className="rounded-sm overflow-hidden">
                ...
                <rect width="12" height="12" fill="url(#skippedLegendSummary)" />
              </svg>
      ...
      <div className="border border-border p-3 lg:p-5">
        <BarChart
          data={chartData}
          xDataKey="loop"
        ...
        >
          ...
          <Bar
            dataKey="skipped"
            fill="url(#skippedPatternSummary)"
            lineCap={3}
            fadedOpacity={0.15}
          />
            ...
            content={({ point }) => {
              ...
              return (
                <div className="flex flex-col gap-2 p-3 min-w-[200px]">
                  <div className="flex items-center gap-2">
                    <span
                      className={`inline-flex items-center px-1.5 py-0.5 text-[9px] font-medium border rounded ${statusStyles}`}
                    >
